...
# insertion (chr1:8000), so A01 doubles as the "unmobilized founder" case.
sanger_clones = pd.read_csv("results/stats/sanger_clone_summary.tsv", sep="\t")
a01 = sanger_clones[sanger_clones["clone"] == "A01"]
check(
    a01.shape[0] == 1
    and not bool(a01["both_sides_confirmed"].iloc[0])
    and a01["forward_status"].iloc[0] == "unmobilized"
    and a01["reverse_status"].iloc[0] == "unmobilized"
    and bool(a01["unmobilized"].iloc[0])
    and bool(a01["ngs_validated"].iloc[0])
    and a01["ngs_side"].iloc[0] == "both",
    "sanger_clone_summary.tsv should show clone A01 at the original insertion "
    "site (unmobilized) from both primers, still validated by a two-sided "
    "NGS site",
)
